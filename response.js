...
function response(room, msg, sender, isGroupChat, replier, imageDB) {
  ...
  var r = { replier: replier, msg: msg, sender: sender, room: room, reply: function (str) {
    this.replier.reply(String(str).trim().encoding());
} };
  if (1) {
    if (1) {
      saveChats(r);
...
function weather(loc) {
  ...
  const forecast2 = forecast.list.map(v=>new Date("T"+v.dt_txt.split(" ")[1]).getHours()+"시 "+Math.round(v.main.temp)+"℃ "+v.weather[0].main).join("\n")
  r.replier.reply(
    loc +
      " 날씨\n" +
      main +
