...
weather_test = {
    ...
    parse : function(baseSoup){ // 파싱
        if(this.isWeather()==false){ return "날씨 검색 실패" } // 날씨 파트가 존재하지 않으면
            ...
            var nowWeather = (String(weatherSoup.select("div > div:nth-child(1) > div > div.card.card_now > div.weather_set_summary")).split("<br>")[0].split('<div class="weather_set_summary"')[1]).trim()
