    ...
    parse : function(baseSoup){ // 파싱
        if(this.isWeather()==false){ return "날씨 검색 실패" } // 날씨 파트가 존재하지 않으면
            ...
            this.resultStr += "(해)" + location + "\n　→ " 
                            ...
                            + "날씨　　　기온 습도 최저 최고\n" ;
            for (i in times){
                this.resultStr += String(times[i].extension("0",2)) + "　"
                                + String(todaysWeather[i].replace(/\s/g,"")).extensionRight(한글공백,5)
                                + String(todaysTemp[i].text()).extension("0",2)+ "　"
                                + String(todaysRain[i]).extensionRight(" ",2) + "　"
                                + todaysWind[i] + "　"
